# Phase 1 — 本机多服务批量操作 + 服务模板（代码完成，待人工验收）

> 状态：**代码完成，待真实环境人工验收**（as-built）。本文档记录已落地的设计与实现，作为后续阶段的基线参考；完成第 6 节人工走查后才能标记为正式验收完成。
...
2. **服务模板**：从 `templates/*.xml` 一键新建服务；本阶段新增 java/nodejs/nacos/mysql/redis/nginx 六个模板，加上原有 Python，当前合计七个。
...
| 新增 6 个模板（当前合计 7 个） | 模板携带平台级依赖/探针字段（这些是 CP 侧数据，Phase 4，**不入 XML**） |
...
- `WinSWManager` 的 7 个方法证明现有功能边界可迁移，但其返回值、路径、超时、错误和下载行为不满足 Agent 合同；Phase 2 只能复用行为认知，必须重构命令执行与持久化边界。
